...
class CoWTx : public WriteTx {
 protected:
  ...
  CoWTx(Allocator* allocator, TxMgr* tx_mgr, const char* buf, size_t count,
        size_t offset)
  ...
  CoWTx(Allocator* allocator, TxMgr* tx_mgr, const char* buf, size_t count,
      ...
      : WriteTx(allocator, tx_mgr, buf, count, offset, state, ticket),
        begin_full_vidx(BLOCK_SIZE_TO_IDX(ALIGN_UP(offset, BLOCK_SIZE))),
        end_full_vidx(BLOCK_SIZE_TO_IDX(end_offset)),
        num_full_blocks(end_full_vidx - begin_full_vidx) {}
};
...
class SingleBlockTx : public CoWTx {
 ...
 public:
  SingleBlockTx(Allocator* allocator, TxMgr* tx_mgr, const char* buf,
                size_t count, size_t offset)
      : CoWTx(allocator, tx_mgr, buf, count, offset),
        local_offset(offset - BLOCK_IDX_TO_SIZE(begin_vidx)) {
    assert(num_blocks == 1);
  }

  SingleBlockTx(Allocator* allocator, TxMgr* tx_mgr, const char* buf,
                size_t count, size_t offset, FileState state, uint64_t ticket)
      : CoWTx(allocator, tx_mgr, buf, count, offset, state, ticket),
        local_offset(offset - BLOCK_IDX_TO_SIZE(begin_vidx)) {
    assert(num_blocks == 1);
  }
  ...
  ssize_t exec() {
    ...
    if (!is_offset_depend) state = blk_table->update(allocator);
...
class MultiBlockTx : public CoWTx {
 ...
 public:
  MultiBlockTx(Allocator* allocator, TxMgr* tx_mgr, const char* buf,
               size_t count, size_t offset)
      : CoWTx(allocator, tx_mgr, buf, count, offset),
